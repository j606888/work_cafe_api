class OpeningHourService::IsOpenNowMap < Service
  ...
  def perform
    ...
    open_store_ids = OpeningHour.where(store_id: @store_ids, open_day: open_day, close_day: open_day)
      ...
      .pluck(:store_id)
    open_to_mid_night_ids = OpeningHour.where(store_id: @store_ids, open_day: open_day, close_day: (open_day + 1) % 7)
      .where("open_time <= :cur_time", cur_time: cur_time).pluck(:store_id)
    ...
    is_open_map = (open_store_ids + open_24_store_ids + open_to_mid_night_ids).uniq.index_with(true)
